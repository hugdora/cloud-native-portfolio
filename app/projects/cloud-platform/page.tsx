      ...
      <div className="container" style={{ maxWidth: "820px" }}>
        ...
        <div style={{ background: "var(--card)", borderLeft: "3px solid var(--accent)", padding: "24px 28px", marginBottom: "52px" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
            {[
              "Eliminated all direct S3 exposure by enforcing CloudFront as the sole origin via Origin Access Control — zero public bucket access",
              "Provisioned HTTPS, CDN delivery, and DNS routing entirely as code with Terraform, making environment rebuilds repeatable in under 10 minutes",
              "Achieved keyless AWS authentication from GitHub Actions via IAM OIDC, removing all long-lived credential risk from the deployment pipeline",
            ].map((result, i) => (
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1px", background: "var(--border)", marginBottom: "52px" }}>
          {[
            { label: "Problem", text: "Traditional public S3 static hosting exposes the origin directly, lacks security controls, and offers no CDN performance. Any direct S3 URL bypasses HTTPS enforcement and rate limiting." },
            { label: "Solution", text: "CloudFront acts as the single secure entry point. The S3 bucket is fully private — accessible only via Origin Access Control. Terraform provisions every resource. GitHub Actions deploys with IAM OIDC (no long-lived keys)." }
          ].map(({ label, text }) => (

...
const stack = [
  "AWS CodePipeline", "AWS CodeBuild", "Amazon S3",
  "React", "GitHub", "buildspec.yml",
  "IAM service roles", "Node.js 18",
];

const decisions = [
  {
    title: "Why CodePipeline over GitHub Actions for this project",
    body: "GitHub Actions keeps the pipeline inside GitHub — the YAML lives in the repo and the runner is GitHub-hosted. CodePipeline keeps everything inside AWS — the pipeline is configured in the AWS console, CodeBuild runs the build in an AWS-managed container, and the deployment is handled by AWS directly. Organisations already deep in the AWS ecosystem often prefer CodePipeline because it integrates natively with CodeBuild, CodeDeploy, and the rest of the AWS developer tooling suite.",
  },
  {
    title: "Why buildspec.yml must be at the repo root",
    body: "CodeBuild looks for buildspec.yml at the root of the source directory by default. The file defines all build phases — install, pre_build, build, post_build — and the artifacts section tells CodePipeline exactly which files to pass to the deploy stage.",
  },
  {
    title: "Why npm ci instead of npm install",
    body: "npm ci installs exactly what is in package-lock.json without resolving. It is faster than npm install on CI because it skips the resolution step, and it is fully reproducible — the same versions every time regardless of when the build runs.",
  },
  {
  ...
  return (
    <main style={{ padding: "56px 0" }}>
      ...
      <div className="container" style={{ maxWidth: "820px" }}>
        ...
        <p style={{ color: "var(--muted)", fontSize: "14px", lineHeight: 1.75, marginBottom: "40px", maxWidth: "620px" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1px", background: "var(--border)", marginBottom: "52px" }}>
          {[
            { label: "Problem", text: "Manual deployments are slow and error-prone. Organisations using AWS tooling exclusively need a delivery pipeline that lives entirely within AWS — not one that depends on GitHub-hosted runners or external CI services." },
            { label: "Solution", text: "AWS CodePipeline detects every GitHub push via webhook and triggers an automated 3-stage pipeline. CodeBuild installs dependencies and builds the React app. The build output is deployed directly to S3 — no manual steps, no server to manage." }
          ].map(({ label, text }) => (

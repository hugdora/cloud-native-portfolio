      ...
      <div className="container" style={{ maxWidth: "820px" }}>
        ...
        <div style={{ background: "var(--card)", borderLeft: "3px solid var(--accent)", padding: "24px 28px", marginBottom: "52px" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
            {[
              "Built a single pipeline that drives two independent deployment tracks (S3/CloudFront and Kubernetes) — reducing pipeline sprawl while increasing delivery coverage",
              "Integrated lint, type-check, test, build, and Trivy security scan as mandatory quality gates before any deployment stage runs",
              "Eliminated all stored AWS credentials by implementing IAM OIDC trust — the pipeline carries no long-lived secrets at any stage",
            ].map((result, i) => (
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1px", background: "var(--border)", marginBottom: "52px" }}>
          {[
            { label: "Problem", text: "Manual deployments are slow, error-prone, and don't scale. Storing AWS credentials as long-lived secrets is a security risk. Running no quality gates means bugs and type errors reach production." },
            { label: "Solution", text: "Every push triggers a full CI run. On success, two independent deploy jobs run in parallel — one for the static path, one for Kubernetes. IAM OIDC means no AWS credentials are stored anywhere in GitHub." }
          ].map(({ label, text }) => (
